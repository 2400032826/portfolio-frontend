  ...
  return (
    <section id="skills" className="py-20 px-6 max-w-6xl mx-auto">
      ...
      <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 mb-2 tracking-tight">
        Technical <span className="text-blue-600">Toolbox</span>
      </h2>
            ...
            {filteredSkills.map((skill, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-mono text-xs font-bold text-slate-800">{skill.name}</span>
                </div>
              );
            })}
          </div>
          ...
          {selectedSkill && (
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900 font-mono">{selectedSkill.name}</h3>
                  <span className="text-xs font-mono font-bold text-blue-600">{selectedSkill.category}</span>
                </div>
              </div>
